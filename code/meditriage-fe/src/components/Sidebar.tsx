...
const Sidebar: React.FC<SidebarProps> = ({ user, onLogout, queueCount }) => {
  ...
  const getActiveTab = () => {
    ...
    if (location.pathname.startsWith('/mdt')) return 'mdt';
    return 'overview';
  };
  ...
  if (user.role === UserRole.DOCTOR) {
    items.splice(2, 0, {
      id: 'mdt', path: '/mdt', label: 'MDT Conference', icon: (
  ...
  const handleNavClick = (path: string) => {
    if (isChatRoute) {
      // Show confirmation before leaving chat
  ...
  return (
    <>
            ...
            <nav ref={navRef} className="relative" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              {items.map(tab => (
                ...
                >
                  ...
                  {tab.id === 'patients' && queueCount !== undefined && queueCount > 0 && (
      ...
      {/* Leave chat confirmation — triggered when clicking sidebar nav during active chat */}
      <ConfirmModal
